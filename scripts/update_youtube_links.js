...
async function run() {
    ...
    // Target range (defaults to Chapter 19: 1651 to 1725)
    let minId = 1651;
    let maxId = 1725;
    if (process.argv[2]) minId = parseInt(process.argv[2]);
    if (process.argv[3]) maxId = parseInt(process.argv[3]);

    const targetEpisodes = rawEpisodes.filter(ep => ep.episode_id >= minId && ep.episode_id <= maxId);
    console.log(`Targeting ${targetEpisodes.length} episodes (${minId} to ${maxId}) for YouTube URL updates...`);
    ...
    const tasks = targetEpisodes.map(ep => async () => {
        ...
        if (fs.existsSync(cacheFile)) {
            ...
            // Only delay when hitting network
            await delay(250);
            try {
        ...
        try {
            ...
            if (ytUrl) {
                ...
                if (oldUrl !== ytUrl) {
                    ...
                    totalUpdated++;
                    console.log(`  [Ep ${ep.episode_id}] Updated: ${oldUrl} -> ${ytUrl}`);
                }
            } else {
                console.log(`  [Ep ${ep.episode_id}] No YouTube link found in topic HTML.`);
            }
        } catch (err) {
    ...
    console.log("Starting batch fetch with concurrency 5...");
    await batchExecute(tasks, 5);

    // Save updated database
    fs.writeFileSync(RAW_FILE, JSON.stringify(rawEpisodes, null, 2), 'utf-8');
    console.log(`Database updated and saved to raw_episodes.json. Total updated in this run: ${totalUpdated}`);
}
